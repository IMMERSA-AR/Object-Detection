...
public class ObjectDetector : MonoBehaviour
{
    ...
    // ── Start: cache references and load model ONCE ────────────────
    // Do NOT start the coroutine here — OnEnable handles that.
    private void Start()
    {
        _cameraAccess = GetComponent<PassthroughCameraAccess>();
        _objectRenderer = GetComponent<ObjectRenderer>();
        ...
        if (!_cameraAccess || !_objectRenderer)
            Debug.LogError("[ObjectDetector] PassthroughCameraAccess or ObjectRenderer not found.");

        LoadModel();

        // If the component starts enabled, OnEnable already fired before Start.
        // Start the coroutine here only if we are currently enabled.
        if (isActiveAndEnabled && _inferenceCoroutine == null)
        {
            Debug.Log("[ObjectDetector] Start() — already enabled, starting inference.");
            _inferenceCoroutine = StartCoroutine(InferenceLoop());
        }
    }

    // ── OnEnable: called every time the component is enabled ───────
    // This is what fires when ExperienceManager does objectDetector.enabled = true
    private void OnEnable()
    {
        // _model is null until Start() runs, so guard against that
        ...
        Debug.Log("[ObjectDetector] OnEnable — starting inference loop.");
        _inferenceCoroutine = StartCoroutine(InferenceLoop());
    }

    // ── OnDisable: stop coroutine cleanly ──────────────────────────
    private void OnDisable()
    {
        if (_inferenceCoroutine != null)
        {
            StopCoroutine(_inferenceCoroutine);
            _inferenceCoroutine = null;
            Debug.Log("[ObjectDetector] OnDisable — inference stopped.");
        }
    }
    ...
    private void LoadModel()
    {
        try
        {
            ...
            _engine = new Unity.InferenceEngine.Worker(_model, backend);
            _modelLoaded = true;
            Debug.Log($"[ObjectDetector] Model loaded on {backend} backend.");
        }
        catch (Exception e)
        {
            Debug.LogError("[ObjectDetector] Failed to load model: " + e.Message);
    ...
    private IEnumerator InferenceLoop()
    {
        Debug.Log("[ObjectDetector] InferenceLoop started.");
        ...
        Debug.Log("[ObjectDetector] InferenceLoop exited.");
    }
    ...
    private IEnumerator PerformInference(Texture texture)
    {
        ...
        if (schedule == null)
        {
            Debug.LogWarning("[ObjectDetector] ScheduleIterable returned null; falling back.");
            _engine.Schedule(inputTensor);
        }
        else
        {
        ...
        Unity.InferenceEngine.Tensor<float> outputTensor =
            _engine.PeekOutput(0) as Unity.InferenceEngine.Tensor<float>;

        outputTensor.ReadbackRequest();
        while (!outputTensor.IsReadbackRequestDone())
            yield return null;

        Unity.InferenceEngine.Tensor<float> cpuTensor =
            outputTensor.ReadbackAndClone() as Unity.InferenceEngine.Tensor<float>;

        int elements = 8400;
        int classes = 80;

        var filteredCoords = new List<float>();
        var filteredLabels = new List<int>();
        var filteredConfs = new List<float>();
        ...
        for (int i = 0; i < elements; i++)
        {
            ...
            if (maxConf > 0.5f)
            {
                float cx = cpuTensor[0 * elements + i];
                float cy = cpuTensor[1 * elements + i];
                float w = cpuTensor[2 * elements + i];
                float h = cpuTensor[3 * elements + i];
    ...
    private bool TryEnsureCameraTexture()
    {
        if (!_cameraAccess || !_cameraAccess.IsPlaying) return false;

        if (_cameraTexture) return true;

        _cameraTexture = _cameraAccess.GetTexture();
        if (_cameraTexture)
        {
            var res = _cameraAccess.CurrentResolution;
            Debug.Log($"[ObjectDetector] Camera ready: {res.x}x{res.y}");
        }
        else
        {
            Debug.LogWarning("[ObjectDetector] Camera texture not ready yet.");
        }
    ...
    private void SaveDebugImage(RenderTexture rt, string fileName)
    {
        ...
        string path = System.IO.Path.Combine(Application.persistentDataPath, fileName);
        System.IO.File.WriteAllBytes(path, bytes);
        Debug.Log($"[DEBUG] Image saved: {path}");
        Destroy(tex);
        RenderTexture.active = null;
    }
}