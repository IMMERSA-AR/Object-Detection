...
namespace LipSync
{
    ...
    public class CustomLipSyncContext : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            // Initialise the predictor here (not in Start) so that FeedAudioClip can be
            // called in the same frame the component is re-enabled.  Unity defers Start()
            // to the next frame after a component is enabled, but Awake() runs exactly once
            // at object creation regardless of enabled state — making the predictor always
            // ready before any external caller touches it.
            if (modelAsset == null)
            {
        ...
        /// <summary>
        /// Zero out all smoothed viseme weights instantly so the mouth returns to its
        /// natural closed position.  Call this right before setting <c>enabled = false</c>
        /// so <see cref="CustomLipSyncMorphTarget"/> sees zero weights and resets all
        /// blend shapes to neutral in its next LateUpdate.
        /// </summary>
        ...
        /// <summary>
        /// Pre-compute the viseme timeline for an incoming AudioClip.
        /// Call this BEFORE the clip is enqueued for playback.
        /// Runs MFCC extraction + batch inference synchronously on the main thread.
        ///
        /// transcript : optional full-clip transcript for text-guided alignment.
        /// streaming  : set true ONLY for live TTS chunks — allows the shared
        ///              _utteranceSeq cursor (set by BeginUtterance) to be consumed.
        ///              Leave false (default) for pre-recorded clips so a stale TTS
        ///              utterance sequence never contaminates them.
        /// </summary>
        public void FeedAudioClip(AudioClip clip, string transcript = null, bool streaming = false)
        {
            ...
            else if (useTextGuidedMFCC && streaming && _utteranceSeq != null
            ...
            {
                // Sequence exhausted/stale for this TTS chunk → raw MFCC.
